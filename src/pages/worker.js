export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/home" && request.method === "GET") {
      try {
        // Fetch all settings from home_settings table
    ...
    // POST /api/home/update - Update home page settings
    if (url.pathname === "/api/home/update" && request.method === "POST") {
      try {
        ...
        const settings = [
          { key: 'video_url', value: data.video_url },
          { key: 'subtitle', value: data.subtitle || '' },
          { key: 'subtitle_link', value: data.subtitle_link || '' },
          { key: 'subtitle_delay', value: String(data.subtitle_delay || 0) },
          { key: 'subtitle_duration', value: String(data.subtitle_duration || 6) },
          { key: 'title', value: data.title || 'ChillaX' },
          { key: 'description', value: data.description || 'Order food, watch TV, and dedicate songs to people you love.' },
        ...
        for (const setting of settings) {
          await env.DB.prepare(
            `INSERT OR REPLACE INTO home_settings (key, value, updated_at) 
             VALUES (?, ?, datetime('now'))`
        ...
        return Response.json({
          ...
          data: {
            ...
            subtitle_link: data.subtitle_link || '',
            subtitle_delay: data.subtitle_delay || 0,
            subtitle_duration: data.subtitle_duration || 6,
            title: data.title || 'ChillaX',
            description: data.description || 'Order food, watch TV, and dedicate songs to people you love.',
        ...
        return Response.json(
          { success: false, message: "Failed to update home settings" },
          { status: 500, headers }
        );
      }
    }
    ...
    if (url.pathname === "/api/dedications" && request.method === "POST") {
      try {
        ...
        // Upload sender photo to R2 if provided
        ...
        // Upload recipient photo to R2 if provided
        ...
        // Upload media file to R2 if provided
        ...
        return Response.json(
          { success: false, message: "Failed to create dedication" },
          { status: 500, headers }
        );
      }
    }
    ...
    // USER: add comment under one media (legacy - keep for MediaComments component)
    if (url.pathname === "/api/media-comment" && request.method === "POST") {
      ...
      await env.DB.prepare(
        `INSERT INTO media_comments (media_id, whatsapp, comment)
         VALUES (?, ?, ?)`
      )
    ...
    // PUBLIC: get comments for one media only (legacy - keep for MediaComments component)
    ...
    // USER: comment on dedication (updated to save actual comment)
    if (url.pathname === "/api/dedications/comment" && request.method === "POST") {
      ...
      try {
        // Insert comment into dedication_comments table
        await env.DB.prepare(
        ...
        // Update comment count in dedications table
        await env.DB.prepare(
          `UPDATE dedications 
    ...
    // ADMIN: manually edit views, reactions, comments (legacy - keep for compatibility)
    ...
    if (url.pathname === "/api/admin/delete-dedication" && request.method === "POST") {
      ...
      try {
        // Delete comments related to the dedication
        await env.DB.prepare(
    ...
    if (url.pathname === "/api/admin/delete-dedication-comment" && request.method === "POST") {
      ...
      try {
        // Get the dedication_id for this comment
