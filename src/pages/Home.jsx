...
// Social Life Comment System
function CommentsSection({ postId, currentUser, isLoggedIn, onRequireAuth }) {
  ...
  const loadComments = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/social/comments?post_id=${encodeURIComponent(postId)}`,
        { cache: "no-store" },
      );
      ...
      devError("Failed to load comments:", error);
    } finally {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const optimisticComment = {
      id: tempId,
      text: text,
      user_name: currentUser.full_name || currentUser.service_provider_name || "User",
    ...
    try {
      const response = await fetch(`${API_URL}/api/social/comments`, {
        method: "POST",
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.error || data.message || "Could not post comment.");
      }
      if (data.comment && isMountedRef.current) {
        setComments((prev) =>
  ...
  }, [commentText, sending, isLoggedIn, currentUser, postId, onRequireAuth]);
      ...
      <div className="comments-list">
        {comments.length === 0 ? (
          ...
          comments.map((comment) => (
            <div key={comment.id} className={`comment-item ${comment.pending ? "is-pending" : ""}`}>
              <img
                src={comment.user_avatar || DEFAULT_LOGO}
                alt=""
...
function Home() {
  ...
  useEffect(() => {
    ...
    const pollReactionCounts = async () => {
      ...
      const ids = postsRef.current.map((post) =>
        String(post.id)).filter(Boolean);
  ...
  const openInboxForPost = useCallback(
    (post) => {
      ...
      if (isSameProviderAsPost(post, user)) {
      ...
      <>
        <main className="home-feed">
          {memoizedPosts.map((post, index) => {
            ...
            const isSocialLife = activeCategory === "social-life";
            ...
            return (
              <ServicePost
                ...
                showComments={isSocialLife}
                isLoggedIn={isLoggedIn}
                user={user}
                onRequireAuthForComment={() => openAuthModal("login")}
              />
            );
          })}
        </main>
...
const ServicePost = memo(function ServicePost({
  ...
  const handleContactClick = useCallback(() => {
    ...
    onContactProvider(post);
  }, [isLoggedIn, currentUser, onRequireAuth, onContactProvider, post]);
  ...
  }, [isLoggedIn, currentUser, onRequireAuth]);
  ...
  const isSocialLife = showComments;
    ...
    >
      ...
      {isSocialLife && (
        <CommentsSection
          ...
          currentUser={user}
          isLoggedIn={isLoggedIn}
          onRequireAuth={onRequireAuthForComment}
        />
      )}
    </section>
  );
  ...
  return (
    <>
      ...
      <div className="tv-public-action-dock" onClick={(event) => event.stopPropagation()}>
        ...
        >
          <Zap size={29} strokeWidth={1.8} aria-hidden="true" />
        </button>
          ...
          {step === "waiting" && (
            <>
              <div className="pin-verification-panel" role="status" aria-live="polite">
                ...
                <strong>
                  {request?.verificationCode || request?.verification_code ||
                    request?.code || "----"}
                </strong>
              </div>
              <p className="pin-reset-intro">
...
.comments-section {
  ...
  background: rgba(2, 12, 28, .72);
  border-radius: 14px;
  border: 1px solid rgba(123, 211, 255, .18);
  box-shadow: 0 0 18px rgba(31, 164, 255, .12);
  backdrop-filter: blur(8px);
  overflow: hidden;
  pointer-events: auto;
}
...
.comment-composer input:focus {
  border-color: rgba(80, 194, 255, .3);
}
...
.comment-error {
  ...
  border-top: 1px solid rgba(255, 51, 79, .1);
}
